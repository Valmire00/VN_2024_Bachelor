...
const [prevDir, newDir, outputFile] = process.argv.slice(2);

const categorizeChanges = (prevTokens, newTokens) => {
  const changes = {
    simpleChanges: [],
    criticalChanges: []
  };
  ...
  for (const key in prevTokens) {
    if (!newTokens[key]) {
      changes.criticalChanges.push(`Deleted: ${key}`);
    }
  }
...
const prevFiles = fs.readdirSync(prevDir).filter(file => file.endsWith('.json'));
const newFiles = fs.readdirSync(newDir).filter(file => file.endsWith('.json'));
...
newFiles.forEach(file => {
  ...
  if (changes.simpleChanges.length > 0 || changes.criticalChanges.length > 0) {
    categorizedChanges += `File: ${file}\n`;
    if (changes.simpleChanges.length > 0) {
      categorizedChanges += `Simple Changes:\n${changes.simpleChanges.join('\n')}\n`;
    }
    if (changes.criticalChanges.length > 0) {
      categorizedChanges += `Critical Changes:\n${changes.criticalChanges.join('\n')}\n`;
    }
  } else {
    categorizedChanges += `File: ${file} - No changes made\n`;
  }
});

fs.writeFileSync(outputFile, categorizedChanges);
